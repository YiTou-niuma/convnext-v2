...
if __name__ == '__main__':
    ...
    # 创建runs文件夹用于保存日志
    runs_dir = 'runs/exp_' + datetime.now().strftime("%Y%m%d-%H%M%S")
    ...
    classes = 6
    # resume ="checkpoints/ConvNext/model_5_93.776.pth"
    resume = None
    CLIP_GRAD = 5.0
    Best_ACC = 0 #记录最高得分
    use_ema=True
    ...
    transform = transforms.Compose([
        ...
        transforms.ToTensor(),
        transforms.Normalize(mean=[0.7167355418205261, 0.7258771061897278, 0.7088801860809326], std=[0.16619576513767242, 0.15309147536754608, 0.23195406794548035])
    ...
    dataset_train = datasets.ImageFolder('E:\code/rice_disease\demo\RDNet_Demo\data/train', transform=transform)
    dataset_test = datasets.ImageFolder("E:\code/rice_disease\demo\RDNet_Demo\data/val", transform=transform_test)
    # 保存类别信息
    with open(os.path.join(runs_dir, 'class.txt'), 'w') as file:
        file.write(str(dataset_train.class_to_idx))
    with open(os.path.join(runs_dir, 'class.json'), 'w', encoding='utf-8') as file:
    ...
    for epoch in range(start_epoch, EPOCHS + 1):
        ...
        # 保存结果到runs文件夹
        with open(os.path.join(runs_dir, 'result.json'), 'w', encoding='utf-8') as file:
            file.write(json.dumps(log_dir))
        # with open(file_dir + '/result.json', 'w', encoding='utf-8') as file:
        #     file.write(json.dumps(log_dir))

        # 保存分类报告
        report = classification_report(val_list, pred_list, target_names=dataset_train.class_to_idx,
                                        output_dict=True)
        with open(os.path.join(runs_dir, f'classification_report_epoch_{epoch}.json'), 'w') as f:
            json.dump(report, f)
        ...
        plt.savefig(os.path.join(runs_dir, "loss.png"))
        plt.close(1)
        ...
        plt.ylabel("acc")
        plt.xlabel("epoch")
        plt.savefig(os.path.join(runs_dir, "acc.png"))
        plt.close(2)
